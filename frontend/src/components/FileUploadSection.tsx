        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs flex flex-col justify-between relative overflow-hidden group hover:border-orange-300 transition-all">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="font-bold text-base text-slate-900">
                  YP Source Excel Data
                </h3>
                <p className="text-xs text-slate-500">
                  Youth Participant records (.xlsx)
                </p>
              </div>
            </div>
          ...
          >
            <input
              ...
              ref={ypInputRef}
              onChange={(e) =>
                e.target.files?.[0] && onUploadYP(e.target.files[0])
              }
              accept=".xlsx,.xls"
              className="hidden"
            />
            ...
            {isUploadingYp ? (
              ...
              <div className="flex flex-col items-center space-y-1">
                ...
                <span className="text-[11px] text-slate-400 underline pt-1">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs flex flex-col justify-between relative overflow-hidden group hover:border-orange-300 transition-all">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="font-bold text-base text-slate-900">
                  MCP Source Excel Data
                </h3>
                <p className="text-xs text-slate-500">
                  Master Crafts Persons Records (.xlsx)
                </p>
              </div>
            </div>
          ...
          >
            <input
              ...
              ref={mcpInputRef}
              onChange={(e) =>
                e.target.files?.[0] && onUploadMCP(e.target.files[0])
              }
              accept=".xlsx,.xls"
              className="hidden"
            />
            ...
            {isUploadingMcp ? (
              ...
              <div className="flex flex-col items-center space-y-2">
                <Upload className="w-6 h-6 text-slate-400 group-hover:text-orange-600 transition-colors" />
                <div className="text-xs text-slate-600 font-medium">
                  Drag & drop MCP `.xlsx` or{" "}
                  <span className="text-orange-600 underline">browse</span>
                </div>
                <span className="text-[11px] text-slate-400">
                  POST /upload/mcp (loads into memory)
                </span>
              </div>
            )}
          </div>
      ...
      <div className="bg-slate-50 border border-slate-200/90 rounded-xl overflow-hidden transition-all">
        ...
        >
          ...
          <div className="flex items-center space-x-1 text-xs font-medium text-slate-500">
            <span>
              {showSchemaGuide ? "Hide Reference" : "View Schema Rules"}
            </span>
            {showSchemaGuide ? (
        ...
        {showSchemaGuide && (
          <div className="p-5 border-t border-slate-200/80 bg-white space-y-4 text-xs text-slate-600">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-slate-50 p-3.5 rounded-lg border border-slate-200 space-y-2">
                ...
                <ul className="space-y-1 font-mono text-[11px] text-slate-700">
                  <li className="flex items-center justify-between">
                    <span>
                      <strong className="text-rose-700">id</strong> (Unique
                      Code/ID)
                    </span>
                  ...
                  <li className="flex items-center justify-between">
                    <span>
                      <strong className="text-rose-700">name</strong>{" "}
                      (Participant Name)
                    </span>
                    <span className="text-[9px] bg-rose-100 text-rose-800 px-1 rounded">
                      REQUIRED
                  ...
                  <li className="flex items-center justify-between">
                    <span>
                      <strong className="text-rose-700">landmark</strong>{" "}
                      (Nearest Cluster/Landmark)
                    </span>
                    <span className="text-[9px] bg-rose-100 text-rose-800 px-1 rounded">
                      REQUIRED
                    </span>
                  ...
                  <li className="flex items-center justify-between">
                    <span>
                      <strong className="text-rose-700">trade</strong> (Fashion
                      Skill / Sector)
                    </span>
                    <span className="text-[9px] bg-rose-100 text-rose-800 px-1 rounded">
                      REQUIRED
                  ...
                  <li className="flex items-center justify-between text-slate-500 font-sans text-[10px]">
                    <span>
                      proceed_flag, gender, pwd, garment_subtype,
                      footwear_subtype, leather_*
                    </span>
              ...
              <div className="bg-slate-50 p-3.5 rounded-lg border border-slate-200 space-y-2">
                ...
                <ul className="space-y-1 font-mono text-[11px] text-slate-700">
                  <li className="flex items-center justify-between">
                    <span>
                      <strong className="text-rose-700">id</strong> (MCP
                      Code/ID)
                    </span>
                    <span className="text-[9px] bg-rose-100 text-rose-800 px-1 rounded">
                      REQUIRED
                  ...
                  <li className="flex items-center justify-between">
                    <span>
                      <strong className="text-rose-700">trade</strong> (Master
                      Specialization)
                    </span>
                    <span className="text-[9px] bg-rose-100 text-rose-800 px-1 rounded">
                      REQUIRED
            ...
            <p className="text-[11px] text-slate-500 italic leading-relaxed">
              * Note: Column resolution uses signature cache → configured alias
              (<code>column_config.json</code>) → exact match → fuzzy match. If
              a required field resolves only via fuzzy matching, the backend
              raises an explicit resolution exception to prevent misattribution.
            </p>
          </div>
        )}
      </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">
                Configure parameters passed to POST /match/run
              </p>
            </div>
          </div>
          {!canRunMatch && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-slate-50/80 p-3 rounded-lg border border-slate-200 space-y-1.5">
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold text-slate-700">
                Hop Limit{" "}
                <span className="text-orange-600">({hopLimit} rounds)</span>
              </label>
            </div>
            <p className="text-[10px] text-slate-500">
              Fallback landmark search depth
            </p>
          ...
          <div className="bg-slate-50/80 p-3 rounded-lg border border-slate-200 space-y-1.5">
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold text-slate-700">
                Match Cap{" "}
                <span className="text-slate-400 font-normal">(MATCH_CAP)</span>
              </label>
              {matchCap !== null && (
                <button
                  type="button"
            ...
            <input
              ...
              placeholder={
                ypLoadedCount
                  ? `No limit (all ${ypLoadedCount})`
                  : "Optional cap (e.g. 100)"
              }
              value={matchCap ?? ""}
              onChange={(e) => {
          ...
          <div className="bg-slate-50/80 p-3 rounded-lg border border-slate-200 space-y-1.5">
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold text-slate-700">
                Round 2 Shortlist{" "}
                <span className="text-orange-600">({shortlistSize})</span>
              </label>
              <span className="text-[10px] text-slate-400">1 - 20 max</span>
            </div>
            <p className="text-[10px] text-slate-500">
              MCP candidates shortlisted in round 2
            </p>
            <div className="flex items-center space-x-2 pt-0.5">
              <input
                ...
                value={shortlistSize}
                onChange={(e) =>
                  onShortlistSizeChange(parseInt(e.target.value, 10))
                }
